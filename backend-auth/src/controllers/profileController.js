...
exports.saveProfile = async (req, res, next) => {
  try {
    ...
    const { university, degree, yearOfStudy, modules, weakSubjects } = req.body;
    ...
    const normalizedWeakSubjects = (weakSubjects || []).map((w) => ({
      name: w.name,
      grade: w.grade || "F",
    ...
    const profile = await AcademicProfile.findOneAndUpdate(
      { user: userId },
      {
        $set: {
          ...
          weakSubjects: normalizedWeakSubjects,
...
exports.updateProfile = async (req, res, next) => {
  try {
    ...
    if (weakSubjects !== undefined) {
      updates.weakSubjects = weakSubjects.map((w) => ({
        name: w.name,
        grade: w.grade || "F",
        marks: Number(w.marks),
      }));
