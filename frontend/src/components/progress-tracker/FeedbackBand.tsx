...
interface Props {
  ...
  subjectName: string;        // ✅ pass subject name from parent (not from CSV/type)
...
export const FeedbackBand: React.FC<Props> = ({
  ...
  const { score, total, band } = result as QuizResult & { band: QuizBand };

  // ✅ pass mark = 50% of total
  const passMark = Math.ceil(total * 0.5);

  const failed = score < passMark;

  const scorePercent = Math.round((score / Math.max(total, 1)) * 100);
  const passPercent = Math.round((passMark / Math.max(total, 1)) * 100);
